...
- **闲鱼价格** = 小程序成本价 + 加价（由 `quote/cost_table.py` + markup 规则决定）
- **首单优惠**：首次使用小程序的手机号，首重仅需 3 元起（续重不变）。这不是"3 元优惠"，是"首重 3 元起"
...
```
先拍下链接不付款 → 我改价 → 付款后自动发兑换码 → 到小程序下单即可
地址和手机号在小程序填写，闲鱼这边不需要提供
```

### 2.3 快递限制

- 闲鱼特价渠道仅支持韵达、圆通、中通、申通
- 顺丰/京东不在特价路线中，回复："闲鱼特价渠道暂不支持顺丰/京东，但在小程序直接下单也比其他平台优惠"
- 不要说"没有顺丰"，而是引导到小程序
...
```
config/config.yaml (YAML) → data/system_config.json (前端可视化配置) → .env / shell 环境变量
```

- `system_config.json` 由前端 UI 管理，运行时通过 `Config._merge_system_config()` 合并，无需手动同步回 YAML
...
- 风控滑块（RGV587）当前优先走 DrissionPage / 指纹浏览器能力链路
...
3. **复购识别**：已有基础逻辑，但无法查询小程序后台判断用户是否真的是首单
4. **滑块自动解决**：依赖 DrissionPage / 指纹浏览器链路，成功率仍受风控环境影响，需要人工兜底
...
- 历史 Docker 相关文件已移除，当前主线不再依赖 Docker 部署
...
**推荐启动方式**：`python -m src.dashboard_server --host 127.0.0.1 --port 8091`（前台）或 `nohup ./venv/bin/python -m src.dashboard_server --host 127.0.0.1 --port 8091 > logs/app.log 2>&1 &`（后台）。
