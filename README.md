...
| 功能 | 描述 |
|------|------|
| **一键在线更新** | Dashboard 内检查更新、下载、备份、重启，全自动完成 |
| **全屏设置引导** | 首次部署自动弹出 SetupWizard，引导完成全部配置 |
| **离线安装包** | macOS / Windows / 通用三版本离线部署包，含全部依赖 |
| **桌面快捷方式** | macOS .command + LaunchAgent / Windows .bat + 开机启动 |
| **敏感词动态替换** | Dashboard 可配置替换规则，回复自动脱敏 |
| **报价安全加价** | Dashboard 可配置加价比例，默认 0%（无额外加价） |
| **Node.js 后端移除** | 三层架构简化为 Python + React 双服务，端口 3001 不再使用 |
...
| 功能 | 描述 |
|------|------|
| **风控滑块自动验证** | RGV587 触发后自动检测 NC/拼图滑块并模拟拖拽，Playwright + OpenCV |
| **CookieCloud 集成** | 浏览器扩展即时同步 Cookie，风控恢复秒级生效，免手动复制 |
| **自动回复引擎增强** | 30+ 意图规则、售后自动转人工、复购引导、系统通知过滤 |
| **商品图片模板重构** | 7 个新视觉模板 + 统一渲染引擎，支持自定义字体/配色 |
| **macOS LaunchAgent** | 一键安装后台守护服务，开机自启动 |
| **前端 TypeScript 迁移** | 全部页面组件迁移至 TypeScript，类型安全 |
