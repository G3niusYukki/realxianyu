...
如果你在闲鱼上卖东西，每天需要重复做很多事情：发布商品、写标题和描述、每天擦亮、调价格、看数据、处理发货。
...
如果你接入了**闲管家开放平台**，工具还可以直接走 API 做：

- 自动改价（优先 API，失败再回退网页操作）
- 自动物流发货（有单号后自动提交）
...
如果你是发实物，并且已经拿到了闲管家的 `AppKey` / `AppSecret`，现在可以让系统直接提交物流发货。
...
python3 -m src.cli orders \
  ...
  --xgj-app-key 你的AppKey \
  --xgj-app-secret 你的AppSecret
```

如果闲管家发货失败，系统会自动降级成“待人工发货任务”，不会直接丢单。
...
- 你可以直接在面板里保存凭证和开关
- 也可以手动重试一次改价或发货
- 订单推送地址可直接填写为：`/api/orders/callback`
