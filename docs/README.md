# Documentation Index

> XianyuFlow | 闲流 (XianyuFlow | 闲流) 完整文档导航

---

## 快速开始

| 文档 | 说明 | 适合人群 |
|------|------|----------|
| [../README.md](../README.md) | 项目概览和核心特性 | 所有人 |
| [../QUICKSTART.md](../QUICKSTART.md) | 5分钟快速启动指南 | 新用户 |
| [../USER_GUIDE.md](../USER_GUIDE.md) | 详细使用说明书 | 终端用户 |

---

## 开发者文档

### 架构与设计

| 文档 | 说明 |
|------|------|
| [ARCHITECTURE.md](ARCHITECTURE.md) | 系统架构设计文档 |
| [API.md](API.md) | HTTP API 接口文档 |
| [DEPLOYMENT.md](DEPLOYMENT.md) | 生产部署完整方案 |

### AI Agent 文档

| 文档 | 说明 |
|------|------|
| [for-agents/index.md](for-agents/index.md) | AI Agent 快速参考指南 |
| [../CLAUDE.md](../CLAUDE.md) | 详细代码约定和仓库参考 |
| [../AGENT_DEPLOYMENT.md](../AGENT_DEPLOYMENT.md) | Agent 部署专用指南（已按当前启动方式修正） |

---

## 项目信息

### 贡献与规范

| 文档 | 说明 |
|------|------|
| [../CONTRIBUTING.md](../CONTRIBUTING.md) | 贡献指南和代码规范 |
| [../SECURITY.md](../SECURITY.md) | 安全最佳实践 |
| [../CHANGELOG.md](../CHANGELOG.md) | 版本更新日志 |

---

## 技术文档

### 集成文档

- [integrations/xianguanjia/](integrations/xianguanjia/) - 闲管家 API 集成
  - [normalized/](integrations/xianguanjia/normalized/) - 标准化 API 文档
    - [open-platform/](integrations/xianguanjia/normalized/open-platform/) - 开放平台接口
    - [virtual-supply/](integrations/xianguanjia/normalized/virtual-supply/) - 虚拟商品供应

---

## 项目记录

### 发布记录

- [release/](release/) - 版本发布说明

### Superpowers 设计文档

- [superpowers/specs/](superpowers/specs/) - 设计规格
- [superpowers/plans/](superpowers/plans/) - 实施计划

---

**项目主页**: https://github.com/G3niusYukki/realxianyu  
**最后更新**: 2026-04-01
