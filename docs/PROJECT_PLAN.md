...
> 状态说明：当前仓库仍以 `src.dashboard_server` 为默认运行主线；`services/` 和 `infra/` 代表服务化与基础设施方向，但不是已经完整替代主线的既成事实。
...
- `services/`：服务化拆分资产（当前以 `gateway-service` 最可用）
- `infra/`：本地集群和基础设施部署资产
...
- 文档与当前 `main` 的真实行为一致。
- 明确区分 `8091` UI、`8000` Gateway、`5173` Vite 开发端口。
