...
@pytest.mark.asyncio
async def test_batch_polish_returns_disabled_status() -> None:
    service = OperationsService(controller=None)
    result = await service.batch_polish(max_items=5)
...
@pytest.mark.asyncio
async def test_batch_polish_disabled_ignores_product_ids(mock_controller) -> None:
    service = OperationsService(controller=mock_controller)
    result = await service.batch_polish(product_ids=["item_a", "item_b", "item_c"], max_items=3)
    assert result["total"] == 0
