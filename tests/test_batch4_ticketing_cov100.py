...
class TestTicketResponder:
    ...
    async def test_compose_post_purchase_reply_success_with_code(self):
        ...
        responder = RuleBasedTicketResponder()
        result = TicketPurchaseResult(success=True, provider="static", ticket_code="TICKET-123")
...
class TestTicketingService:
    ...
    async def test_prepare_listing_from_screenshot(self):
        ...
        selection = TicketSelection(cinema="万达影城", showtime="19:00", seat="A1", count=2, confidence=0.9)
        quote = TicketQuote(
            provider="static",
            face_value=50.0,
            channel_price=45.0,
