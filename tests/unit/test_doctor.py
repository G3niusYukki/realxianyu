...
def test_extra_checks_web_ui_port_suggestion_mentions_vite_dev_only(monkeypatch) -> None:
    """Verify the Web UI port check suggestion text is correct in the source."""
    # Verify the suggestion text is present in the doctor.py source.
    # This is a code inspection test since the suggestion is only set when
    # runtime != "lite", which depends on the environment.
    import inspect

    source = inspect.getsource(doctor._extra_checks)
    assert "npm run dev" in source
    assert "生产部署无需监听 5173" in source
    assert "Vite 前端开发代理" in source
