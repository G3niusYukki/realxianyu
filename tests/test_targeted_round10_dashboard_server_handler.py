...
def test_do_get_api_statuses_and_error_fallbacks(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    h.do_GET()
    # Handler returns error but route is matched, status depends on handler logic
    h.module_console.status.assert_called_once()
    ...
    h3.do_GET()
    h3.module_console.logs.assert_called_once_with(target="presales", tail=50)
...
def test_do_get_sqlite_and_generic_exception_handlers() -> None:
    ...
    h.do_GET()
    h.repo.get_summary.assert_called_once()
...
def test_do_post_routes_and_status_codes() -> None:
    ...
    h3.do_POST()
    h3.mimic_ops.parse_cookie_text.assert_called_once()
    ...
    h6.do_POST()
    h6.mimic_ops.save_markup_rules.assert_called_once()
    ...
    h7.do_POST()
    h7.mimic_ops.test_reply.assert_called_once()
...
def test_do_post_not_found_and_exception_path() -> None:
    ...
    h2.do_POST()
    h2._send_json.assert_called_once()
