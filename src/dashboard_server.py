...
class DashboardHandler(BaseHTTPRequestHandler):
    ...
    def _send_cors_headers(self) -> None:
        ...
        if not origin or not _is_allowed_dashboard_origin(origin):
            return
    ...
    def _ensure_api_request_access(self, method: str, path: str) -> bool:
        allowed, status, code = _check_api_request_access(
            method=method,
            path=path,
        ...
        if allowed:
            return True
        message = "请求来源不被允许" if code == "FORBIDDEN_ORIGIN" else "Dashboard API token 无效"
        self._send_json(_error_payload(message, code=code or "FORBIDDEN"), status=status)
        return False

    def do_OPTIONS(self) -> None:
        parsed = urlparse(self.path)
        path = parsed.path
        if not self._ensure_api_request_access("OPTIONS", path):
            return
    ...
    def do_GET(self) -> None:
        ...
        try:
            if not self._ensure_api_request_access("GET", path):
                return
    ...
    def do_PUT(self) -> None:
        ...
        path = parsed.path
        try:
            if not self._ensure_api_request_access("PUT", path):
                return
            # --- Route dispatch (decorator-registered routes) ---
            ctx = RouteContext(
                _handler=self,
    ...
    def do_DELETE(self) -> None:
        ...
        path = parsed.path
        try:
            if not self._ensure_api_request_access("DELETE", path):
                return
            ctx = RouteContext(
                _handler=self,
                path=path,
    ...
    def do_POST(self) -> None:
        ...
        try:
            if not self._ensure_api_request_access("POST", path):
                return
            if path.startswith("/cookie-cloud/") or path == "/cookie-cloud":
